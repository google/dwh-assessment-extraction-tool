...
gcloud compute instances create "${TD_HOST}" --project="${GCP_PROJECT}" --zone=us-central1-a --machine-type=n2-standard-2 \
--network-interface=subnet=default,no-address --maintenance-policy=MIGRATE --service-account="${GCP_SERVICE_ACCOUNT}" \
--scopes="${GCP_SCOPES}" --create-disk=auto-delete=yes,boot=yes,device-name="${TD_HOST}",image="${GCP_IMAGE}",mode=rw,size=300,type=projects/"${GCP_PROJECT}"/zones/us-central1-a/diskTypes/pd-balanced \
