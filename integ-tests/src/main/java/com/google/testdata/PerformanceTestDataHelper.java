...
public final class PerformanceTestDataHelper {
  ...
  public static void generateTables(Connection connection, int tablesCount) throws SQLException {
    ...
    final String performancePrecondition1 = getSql(SQL_TESTDATA_BASE_PATH + "users_data.sql");
    final String performancePrecondition2 =
        getSql(SQL_TESTDATA_BASE_PATH + "performance_data_1.sql");
    final String performancePrecondition3 =
        getSql(SQL_TESTDATA_BASE_PATH + "performance_data_2.sql");
    final String performanceQuery1 = getSql(SQL_TESTDATA_BASE_PATH + "columns_data_2.sql");
    ...
    for (int i = 0; i < USER_COUNT; i++) {
      ...
      String createUser =
          format(performancePrecondition1, userData.getUsername(), userData.getPassword());
      String createDatabase = format(performancePrecondition2, dbName, SIZE, SIZE, SIZE);
      String grantCreateTable = format(performancePrecondition3, dbName, userData.getUsername());
      preconditionQueries.addAll(Arrays.asList(createUser, createDatabase, grantCreateTable));

      for (int query = 0; query < TABLES_PER_USER; query++) {
        String tableName = getRandomTableName();
        String createTable = format(performanceQuery1, dbName, tableName);
